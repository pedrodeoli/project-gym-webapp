...
# Método POST para criar um novo instrutor
...
@instrutores_blueprint.route("/instrutores/<id>/editar")
...
def mostra_instrutor(id):
    instrutor_atividades = conector_instrutor.get_activities(id)
    instrutor = conector_instrutor.get_one(id)
    return render_template("instrutores/mostrar.html",
                            instrutor=instrutor, instrutor_atividades=instrutor_atividades,
                             title="Detalhes do Instrutor")

# Rota para deletar um instrutor
@instrutores_blueprint.route("/instrutores/<id>/deletar")
